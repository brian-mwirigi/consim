...
**Experiments.** 5,500 primary runs: 50 seeds × 5 topologies × 5 noise levels ($\sigma \in \{0.04, 0.08, 0.12, 0.20, 0.30\}$), at sizes 12 and 24 (1,250 each). Separate size-48 sweep (250 runs) with all metrics. GoL experiments at size 24.
...
**Finding 4: Self-determination hurts self-knowledge on high-K grids.** On Moore grids (K=8), temporal persistence (T) correlates with self-prediction at r = +0.70, while causal efficacy (E) anti-correlates at r = −0.71. The two metrics are anti-correlated with each other: r(T, E) = −0.82. Agents whose trajectories are externally driven (low E) but whose self-models are stable over time (high T) achieve the best self-prediction. The mechanism: 8 neighbors average into a predictable input signal. Agents driven by that signal have predictable futures. Agents driven by their own nonlinear dynamics are harder for themselves to forecast. Self-determination is noise from the self-model's perspective. Hex (K=6) shows the same pattern more weakly. Von Neumann (K=4) and random show no effect.
